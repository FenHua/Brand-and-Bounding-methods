...
template<class T>T Queue<T>::Top()const
{
	if (IsEmpty())
	{
...
template<class T>Queue<T>&Queue<T>::AddLeft(const T&x)
{
	if (IsFull)
		cout << "queue:no memory" << endl;
	else
...
template<class T>void Queue<T>::Output(ostream&out)const
{
...
template<class T>ostream&operator<<(ostream&out, const Queue<T>&x)
{
	x.Output(out);
	return out;
}