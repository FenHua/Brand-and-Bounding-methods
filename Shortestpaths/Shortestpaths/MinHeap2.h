...
template<class T>void MinHeap<T>::output(T a[], int n)
{
	for (int i = 1; i <= n; i++)
	{
		cout << a[i] << " "£»
...
template<class T>MinHeap<T>::MinHeap(int maxheapsize)
{
	maxsize = maxheapsize;
...
template<class T>void MinHeap<T>::Initialize(T x[], int size, int ArraySize)
{
	delete[]heap;
	heap = x;
	currentsize = size;
...
template<class T>void MinHeap<T>::Deactivate()
{
	//deactivate Ê¹Í£Ö¹¹¤×÷
	heap = 0;
}